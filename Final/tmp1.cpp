...
void matMultiplyCPU(const matCsr& csr, const mat& B, mat& C, int startRow, int endRow) {
    int k = B.m;
...
void matMultiply(const matCsr& csr, const mat& B, mat& C, int k) {
    int n = csr.n;
    int m = csr.m;
    int numRowsCPU = n / k;
    ...
    // CPU part
    thread cpuThread(matMultiplyCPU, ref(csr), ref(B), ref(C), 0, numRowsCPU);
...
int main() {
    ...
    int splitFactor = 4; // k value for CPU/GPU split
