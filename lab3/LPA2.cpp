//g++ -fopenmp -mavx2 -lpthread -o LPA2.exe LPA2.cpp
#include <iostream>
#include <vector>
#include <cstdlib>
#include <ctime>
...
int calMaxRes(mat &Y,int i,int n_classes)
{
    int max_index = 0;
    double max_value = Y.v[i][0];
    int j;
    for (int j = 0; j+4 < n_classes; j+=4) 
    {
        __m256d a_vec=_mm256_loadu_pd(&Y.v[i][j]);
        __m256d max_vec=_mm256_max_pd(a_vec, max_vec);
        __m256d temp_vec=_mm256_permute2f128_pd(max_vec, max_vec, 0x1);
        max_vec=_mm256_max_pd(max_vec, temp_vec);
        temp_vec=_mm256_shuffle_pd(max_vec, max_vec, 0x5);
        max_vec=_mm256_max_pd(max_vec, temp_vec);
        double max_value_temp = _mm256_cvtsd_f64(max_vec);
        if (max_value_temp > max_value) {
            max_value = max_value_temp;
            max_index = j;
        }
    }
    for (int j=n_classes-n_classes%4;j<n_classes;j++)
    {
...
void labelPropagation(matCoo& X, mat& y_label, mat& y_pred, mat& y_res, int mode = 0, double alpha = 0.5, int max_iter = 1000,int sort_thread=8,int sort_mode=1)
{
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        switch (mode)
        {
        default:matMultiplyOmpAvx(W, Y, Y_new, 15); break;
        }
    ...
    #pragma omp parallel for schedule(static)
    for (int i = 0; i < n_samples; ++i) {
        y_pred.v[i][0] =calMaxRes(Y,i,n_classes);
    }
