...
void matMultiply_mpi1(matCsr& x1, mat& x2, mat& res) {
	int n = x1.n, m = x2.m;
	int rank, size;
...
void matMultiply_mpi2(matCsr& x1, mat& x2, mat& res) {
	int n = x1.n, m = x2.m;
	int rank, size;
	MPI_Comm_rank(MPI_COMM_WORLD, &rank);
	MPI_Comm_size(MPI_COMM_WORLD, &size);
...
void matMultiply_cyclic(matCsr& x1, mat& x2, mat& res, int rank, int size) {
	int n = x1.n, m = x2.m;
...
int main(int argc, char** argv) {
	...
	for (int n0 = 128; n0 <= 2048; n0 <<= 1)
	{
		for (int t0 = n0 * n0 / 100; t0 <= n0 * n0 / 100; t0 <<= 1)
		{
			...
			for (int i = 1; i <= 10; ++i) matMultiply_mpi1(csr, dense, result);
			auto end = high_resolution_clock::now();
			auto duration = duration_cast<milliseconds>(end - start).count();

			
			if (rank == 0) {
				cout << 5 << " " << n0 << " " << t0 << " " << duration << endl;
				//matMultiply_avx_omp(csr, dense, result);
			}


			start = high_resolution_clock::now();
			for (int i = 1; i <= 10; ++i) matMultiply_mpi2(csr, dense, result);
			end = high_resolution_clock::now();
			duration = duration_cast<milliseconds>(end - start).count();

			if (rank == 0) {
				cout << 6 << " " << n0 << " " << t0 << " " << duration << endl;
				//matMultiply_avx_omp(csr, dense, result);
			}

			start = high_resolution_clock::now();
			for (int i = 1; i <= 10; ++i) matMultiply_cyclic(csr, dense, result,rank,size);
			end = high_resolution_clock::now();
			duration = duration_cast<milliseconds>(end - start).count();

			if (rank == 0) {
				cout << 7 << " " << n0 << " " << t0 << " " << duration << endl;
				//matMultiply_avx_omp(csr, dense, result);
			}
		}
	}
	...
	//		if (rank == 0) {
	//			cout << 1<< " " << n0 << " " << t0 / 1024.0 << " " << duration << endl;
	//			//matMultiply_avx_omp(csr, dense, result);
	//		}
	...
	//		//if (rank == 0) {
	//		//	cout << 6 << " " << n0 << " " << t0 / 1024.0 << " " << duration << endl;
	//		//	//matMultiply_avx_omp(csr, dense, result);
	//		//}
	...
	//		//if (rank == 0) {
	//		//	cout << 7 << " " << n0 << " " << t0 / 1024.0 << " " << duration << endl;
	//		//	//matMultiply_avx_omp(csr, dense, result);
	//		//}
